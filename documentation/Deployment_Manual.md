...
## 5A. One-Time PostgreSQL Migration for Older Databases

Skip this section for a brand-new empty database.

Run this section once if the target VM database was created before `User.id` was converted from string ids to sequence-backed `bigint` ids.
...
After this one-time migration, normal application startup can proceed with `./gradlew bootRun` or the deployed JAR.
